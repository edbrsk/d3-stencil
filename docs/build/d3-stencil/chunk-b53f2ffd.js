...
const resize = (graph = { axisData: false }) => {
    const getAxisData = (graphMeta) => {
        if (graph.axisData) {
            const labels = getResponsiveLabels(graphMeta);
            return {
                labels,
                range: labels.map((_, index) => index * (graphMeta.width / (labels.length - 1)))
            };
        }
        return null;
    };
    const getResponsiveLabels = (graphMeta, margin = 70) => {
        const separations = Math.floor(graphMeta.width / margin);
        const ticks = separations > graphMeta.graphData.labels.length
            ? 1
            : Math.ceil((graphMeta.graphData.labels.length - 1) / separations);
        const originalGraphLabels = graphMeta.graphData.labels;
        const labels = originalGraphLabels.filter((_, index) => index === 0 || index % ticks === 0);
...
var Formats;
(function (Formats) {
    Formats["PERCENTAGE"] = "PERCENTAGE";
    Formats["GROUPED_TWO_DIGITS"] = "GROUPED_TWO_DIGITS";
    Formats["GROUPED_THOUSANDS_TWO_DIGITS"] = "GROUPED_THOUSANDS_TWO_DIGITS";
    Formats["CURRENCY"] = "CURRENCY";
    Formats["DAY_AND_MONTH"] = "DAY_AND_MONTH";
    Formats["SHORT_MONTH"] = "SHORT_MONTH";
    Formats["LARGE_MONTH"] = "LARGE_MONTH";
    Formats["ANY"] = "ANY";
})(Formats || (Formats = {}));
...
const formats = {
    ...
    [Formats.CURRENCY]: (data, CURRENCY) => data.toLocaleString(navigator.language, {
        style: 'currency',
        currency: CURRENCY
    }),
...
const DEFAULT_GRAPH_DATA_PIE = {
    ...
    colors: [],
    data: [],
    hasDataMethod: (graphDataMerged) => hasDataIsNotempty(graphDataMerged)
};
const DEFAULT_GRAPH_DATA_BAR = {
    labels: [],
    barChartOptions: {
        axis: {
            x: {
...
const DEFAULT_GRAPH_DATA_LINE = {
    ...
    lineAnnotationsChartOptions: {
        ...
        hasDataMethod: (graphDataMerged) => hasDataValidOnAnnotationsChart(graphDataMerged)
    },
    styles: {
        width: '0',
        height: '0',
...
const DEFAULT_GRAPH_DATA_BCG = {
    ...
    colors: [],
    data: [],
    hasDataMethod: (graphDataMerged) => hasDataBCGMatrixIsNotEmpty(graphDataMerged)
};
const DEFAULT_LEGEND_DATA = {
    labels: [],
    colors: [],
    type: 'horizontal',
    styles: {
...
const formatter = (type, data, currency = null) => type === Formats.CURRENCY
    ? formats[type](data, currency)
    : formats[type](data);

const circularFind = (array, index) => {
    const remainder = index % array.length;
    return array[remainder === 0 ? 0 : remainder];
};

const initTooltipIfExists = (chartElement, key) => {
    const tooltip = {
...
const initLegendIfExists = (chartElement, key, callback) => {
    const legend = {
        element: chartElement.getElementsByClassName(key)[0],
        component: null
    };
...
const hasDataIsNotempty = (graphDataMerged) => graphDataMerged.data.length > 0 && graphDataMerged.labels.length >= 1
    ? true
    : THROW_ERROR();
const hasDataValidOnAnnotationsChart = ({ lineAnnotationsChartOptions }) => {
    if (!(lineAnnotationsChartOptions.increaseHeight > 0 &&
...
const hasDataBCGMatrixIsNotEmpty = (graphDataMerged) => (graphDataMerged.data.length > 0 ? true : THROW_ERROR());

export { objectAssignDeep as a, select as b, event as c, max as d, ordinal as e, linear$1 as f, axisBottom as g, axisLeft as h, line as i, resize as j, initTooltipIfExists as k, initLegendIfExists as l, formatter as m, circularFind as n, DEFAULT_GRAPH_DATA_LINE as o, DEFAULT_GRAPH_DATA_BCG as p, band as q, DEFAULT_GRAPH_DATA_BAR as r, DEFAULT_LEGEND_DATA as s, Formats as t, arc as u, pie as v, DEFAULT_GRAPH_DATA_PIE as w, color as x, interpolateNumber as y, interpolateRgb as z, interpolateString as A, interpolateTransformSvg as B, namespace as C, matcher$1 as D, selector as E, selectorAll as F, selection as G, interpolateTransformCss as H, styleValue as I };
